defmodule FsmDslTest do
  use ExUnit.Case


  defmodule Fsm do
    use FsmDsl, register: {:local, :test_fsm},
                initial_state: :start,
                init_params: [:p1, :p2]
  ...
  test "next_state returns the correct info" do
    assert FsmDsl.next_state(:state1, [1,2]) == {:next_state, :state1, [1,2] }
  end

  test "next_state returns the correct info with timeout" do
    assert FsmDsl.next_state(:state1, [1,2], 1234) == {:next_state, :state1, [1,2], 1234 }
  end
